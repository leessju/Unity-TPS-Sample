...
public class Gun : MonoBehaviour
{
    ...
    public void Setup(PlayerShooter gunHolder)
    {
        ...
        this.excludeTarget = gunHolder.excludeTarget;
    }
    ...
    public bool Fire(Vector3 aimTarget)
    {
        if(state == State.Ready && Time.time > lastFireTime + timeBetFire) 
        {
            ...
            //Quaternion.AngleAxis(yError, Vector3.up) => Vector3.up 기준으로 yError 만큼 회전한다. 
            //Quaternion.AngleAxis(yError, Vector3.up) * fireDirection => fireDirection 만큼 회전한다.
    ...
    private void Shot(Vector3 startPoint, Vector3 direction)
    {
        ...
        if(Physics.Raycast(startPoint, direction, out hit, fireDistance, ~excludeTarget)) {
            ...
            hitPosition = startPoint + direction * fireDistance; // 사정거리 
        }
